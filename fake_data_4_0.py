...
def insert_queue_tokens(cursor, conn):
    ...
    for i in range(1, 101):
        ...
        rows.append((name, token_number, status, issued_at, called_at))

    cursor.executemany(
        "INSERT INTO queue_tokens (patient_name, token_number, status, issued_at, called_at) VALUES (%s,%s,%s,%s,%s)",
        rows
    )
    conn.commit()
